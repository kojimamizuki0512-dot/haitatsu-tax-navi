...
type Props = {
  ...
  kind: InboxKind;
  onClose: () => void;
  // Web: name/type/text/uri（画像はobjectURL） / ネイティブ: name/uri
  onPicked: (files: { name: string; type?: string; text?: string; uri?: string }[]) => void;
};

// 日本語判定の簡易スコア（Shift_JIS判定に流用）
...
export default function UploadModal({ visible, kind, onClose, onPicked }: Props) {
  const accept = useMemo(() => {
    switch (kind) {
      ...
      case 'statement': return '.csv,.tsv,.txt,.pdf,.png,.jpg,.jpeg,.webp';
      case 'bank':      return '.csv,.tsv,.txt';
      case 'entry':     return '.png,.jpg,.jpeg,.webp,.mp4,.mov,.m4a';
    }
  }, [kind]);

  const title = useMemo(
    () => (kind === 'work' ? '稼働（日次)' : kind === 'statement' ? '明細（CSV/TSV/PDF）' : kind === 'bank' ? '銀行（CSV/TSV）' : '入口（写真/動画/音声）'),
    [kind]
  );
  ...
  const captureCameraNative = async () => {
    ...
    const cap = await ImagePicker.launchCameraAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, quality: 0.8 });
  ...
  return (
    <View style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.28)', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <View style={{ width: '100%', maxWidth: 520, backgroundColor: '#fff', borderRadius: 20, padding: 20, shadowColor: '#000', shadowOpacity: 0.12, shadowRadius: 18, shadowOffset: { width: 0, height: 8 } }}>
        <Text style={{ fontSize: 18, fontWeight: '800', marginBottom: 8 }}>{title} のアップロード</Text>
        <Text style={{ color: '#666', marginBottom: 16 }}>
          {Platform.OS === 'web' ? '画像はタイル内にプレビューされます（CSVはその場集計／PDFは受付のみ）。' : '端末から選ぶか、カメラ撮影できます。'}
        </Text>
        ...
        {Platform.OS === 'web' ? (
                ...
                const enriched = await Promise.all(files.map(async (f: any) => {
                  ...
                  if (isTextish) {
                    ...
                    try {
                      ...
                      text = jpScore(sjis) > jpScore(utf8) ? sjis : utf8;
                    } catch {
                      text = utf8;
                    }
                  } else {
                    // 画像などはプレビュー用の objectURL を返す
                    if (type.startsWith('image/') || /\.(png|jpg|jpeg|webp)$/i.test(name)) {
                      uri = URL.createObjectURL(f);
                    }
                  }

                  return { name, type, text, uri };
                }));
                onPicked(enriched);
                onClose();
              }}
          ...
          <View style={{ gap: 12 }}>
            <Pressable
              ...
              style={({ pressed }) => ({ paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12, backgroundColor: pressed ? '#efefef' : '#f5f5f6' })}
            >
            ...
            {(kind === 'work' || kind === 'entry') && (
              <Pressable
                ...
                style={({ pressed }) => ({ paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12, backgroundColor: pressed ? '#e6f3ff' : '#eef6ff' })}
              >
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 12 }}>
          <Pressable
            onPress={onClose}
            style={({ pressed }) => ({ paddingHorizontal: 14, paddingVertical: 10, borderRadius: 10, backgroundColor: pressed ? '#efefef' : '#f5f5f6' })}
          >
